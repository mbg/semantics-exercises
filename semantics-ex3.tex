...
\begin{parts}
	...
	\part[2] Suppose that you want to write a pre-processor for your language which takes data type declarations in an \emph{arbitrary} order and re-orders them in order of dependency to fulfil the restriction stated above. For example, if the pre-processor is given
    \begin{displaymath}
    ...
    \end{displaymath}
    since the definition of $\mathit{Foo}$ depends on $\mathit{Bar}$. Propose an algorithm for this purpose.
    \emph{Hint}: Think back to Part 1A Algorithms. \droppoints 
...
\begin{mathpar}
	...
	\inferrule*[right=\color{black}E-CONC-LEFT]
	{ P \longrightarrow Q }
...
%\end{array}
%\end{displaymath}
These semantics do not include cases for all forms of terms in the $\pi$-calculus. This is not required because the remaining rules can be addressed by \emph{structural congruence}. We will use $\simeq$ to denote the structural congruence relation which we will define in the following exercises.
\begin{parts}
	...
	\part[2] Two processes $P$ and $Q$ are structurally congruent if they are identical up to $\alpha$-conversion (\emph{i.e.} identical up to renaming). Extend the structural congruence relation $\simeq$ with this property. \droppoints
